...
class DataProcessor:
    ...
    def select_cd(table: list, cd_idx: int) -> DC.CD:
        ...
        try:
            ...
            print('ID is not an Integer, please use a whole number only')
            print(e.__doc__)
        for row in table:
            if row.cd_id == cd_idx:
                return row
            #TODO Find Root cause why Getting false Messages from this else statement. Error Currently Suppressed
            #else:
                #print('This CD is not in the database')
    ...
    def add_track(track_info: tuple, cd: DC.CD) -> None:
        ...
        try:
            ...
            print('Position must be an integer', e, e.__doc__, type(e), sep='\n')
